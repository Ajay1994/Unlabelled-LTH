...
########################################################################################
# python lottery_ticket.py  --manualSeed 42  --resume results/20/pruned.pth.tar --model results/0/init.pth.tar --save_dir results/20/
...
from utils.misc import get_conv_zero_kernel, get_conv_zero_param
...
parser.add_argument('--schedule', type=int, nargs='+', default=[4, 6, 15],
...
def main():
    ...
    model_ref = models.resnet50(pretrained=True)
    num_ftrs = model_ref.fc.in_features
    model_ref.fc = nn.Sequential(
        nn.Linear(num_ftrs, num_classes), nn.Sigmoid())
    ...
    dev_loss, dev_AUCs, dev_acc = test(devloader, model, criterion, 0, use_cuda)
    print("------------ Before training model accuracy : {} -------------".format(dev_acc))
    # ############################### Train and val ###############################
    all_result = {}
